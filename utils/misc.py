...
def prep_experiment(args, parser):
    ...
    args.last_record = {}
    if not args.restore_optimizer:
        if ['gtav', 'synthia'] == args.dataset:
            if 'DeepR50V3PlusD' in args.arch:
                args.max_epoch = 40
            elif 'DeepR101V2D' in args.arch:
                args.max_epoch = 94
        elif ['gtav', 'gtav'] == args.dataset:
            args.max_epoch = 30
        elif ['gtav', 'idd', 'synthia'] == args.dataset:
            args.max_epoch = 92
        else:
            args.max_epoch = 150
...
def evaluate_eval(args, net, optimizer, scheduler, val_loss, hist, dump_images, writer, epoch=0, dataset_name=None, dataset=None, curr_iter=0, optimizer_at=None, scheduler_at=None, save_pth=True):
    ...
    if save_pth:
        ...
        torch.save(savedict, last_snapshot)
